...
function TerrainCanvas({
  ...
  useEffect(() => {
    ...
    const heightSampleWidth = meshSegments + 1;
    const heightSampleHeight = meshSegments + 1;
    ...
    scene.add(grid);
    const geo = new THREE.PlaneGeometry(8, 8, meshSegments, meshSegments);
    ...
    const updateGeometry = () => {
      ...
      for (let i = 0; i < pos.count; i++)
        pos.setY(
          i,
          (heightField[i] - current.heightBaseline) *
            current.heightWorldScale *
            exaggeration,
        );
    ...
    const addWaypoint = (event) => {
      ...
      if (state.current.heightField) {
        const column = Math.min(
          ...
          Math.max(
            0,
            Math.round(((point.x + 4) / 8) * (heightSampleWidth - 1)),
          ),
        );
        const row = Math.min(
          ...
          Math.max(
            0,
            Math.round(((4 - point.z) / 8) * (heightSampleHeight - 1)),
          ),
        );
        const heightValue =
          state.current.heightField[row * heightSampleWidth + column];
        point.y =
          (heightValue - state.current.heightBaseline) *
          state.current.heightWorldScale *
  ...
  useEffect(() => {
    ...
    for (let i = 0; i < p.count; i++)
      p.setY(
        i,
        (heightField[i] - (state.current.heightBaseline ?? TERRAIN_BASELINE)) *
          (state.current.heightWorldScale ?? HEIGHT_WORLD_SCALE) *
          exaggeration,
      );
    p.needsUpdate = true;
          ...
          >
            ...
            <div className="viewport-footer">
              ...
              <div className="footer-note">
                <Eye size={14} /> {sample.meshResolution || HEIGHT_SAMPLE_WIDTH}²
          ...
          <aside className="inspector">
            ...
            <div className="control-section upload-switcher">
              ...
              <p className="upload-copy">
                ...
                heights. PNG/JPG: estimates relative structure from RGB depth and
                semantic classes.
              </p>
              <input
                ref={uploadFileRef}
                ...
                <>
                  ...
                  {uploadMeta.geo?.georeferenced && (
                    <div className="geo-output">
                      ...
                      <span>
                        {uploadMeta.geo.width}×{uploadMeta.geo.height} native grid ·{" "}
                        {uploadMeta.geo.res_m?.[0]?.toFixed(2)} m/px
                      </span>
                  ...
                  {!uploadMeta.geo?.georeferenced && uploadMeta.terrainGlbUrl && (
                    <div className="geo-output">

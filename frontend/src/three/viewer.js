// Ported unchanged from viewer/web/js/terrain.js (renamed to avoid clashing
// with the Python viewer/terrain.py module). Same imperative API: initViewer
// returns an object with loadScene/setExaggeration/setColormap/setMode/
// setNavMode, driven by the Viewer3D React component via a ref.
...
// imagery's own resolution now. ~18.9M triangles for one static terrain
// patch; fine on a discrete GPU, first knob to turn down for lower-end
// devices.
 ...
 *
 * Depth ships as 16 bits packed into a PNG's R/G channels (rg16) -- decoding
 * to float here rather than sampling the packed bytes in GLSL means the
 * shader stays unchanged and a packed-byte texture is never linearly
 * filtered (interpolating the high byte alone produces garbage between texels).
 */
...
/**
 * @param opts.segments mesh grid resolution (default 3072). Displacement is
 *   sampled per-vertex from the depth texture, so a coarse mesh smooths away
 *   real detail present in the data. Geometry is built once and reused for
 *   every scene swap (only the textures change).
 */
export function initViewer(canvas, opts = {}) {
  ...
  function heightAt(u, v) {
    ...
    const m = state.meta;
    // Ranges come from the ACTIVE variant, not always from meta: reading
    // meta's here would make the CPU probe disagree with the GPU surface.
  ...
  renderer.setAnimationLoop(() => {
    ...
    if (navMode === 'orbit') {
      ...
      // Clamp above terrain by sampling the height field.
  ...
  async function loadScene(base, opts = {}) {
    ...
    // both must agree with the RGB texture's convention (row 0 = top = v=1).
    tex.flipY = true;
  ...
  return {
     ...
     *  (headless browsers, embedded frames, missing user gesture). */
    setNavMode: (m) => {
      if (m === 'fly') {
        try {
          fly.lock();

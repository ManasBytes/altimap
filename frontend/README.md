...
The full-height scene stage uses a compact floating inspector panel. Use the scene switcher in the inspector to move through the 18 available locations. Display textures are rendered at 2048 × 2048, while displacement keeps the native 1024 × 1024 height grid.

RGB texture mode and `0.3×` vertical exaggeration are the defaults. Geometry uses a dedicated grayscale displacement map derived from AGL values with percentile clipping, median filtering, and Gaussian smoothing so tree canopies and roof noise do not become needle-like spikes. During manual flight, use `W/A/S/D` to move horizontally and `Q/E` to lower or raise the camera. Movement is slow, time-based, and eased; the scene remains draggable with Three.js OrbitControls when no flight key is pressed.
